...
load_conda(
    ...
    install_mamba = USE_MAMBA, # use True to use mamba, which a faster drop-in replacement for conda
)
...
conda_create(
    ...
    quiet = False,  # use True to hide conda output
    use_mamba = USE_MAMBA, # use True to use mamba, which a faster drop-in replacement for conda
)
